...
parser.add_argument('--model', type=str, default='MSCN')
...
parser.add_argument('--data_path', type=str,
                    default='/root/MF-Net-1202/data/Multivariate_arff')  # G:\桌面\Multivariate2018_arff\Multivariate_arff ./data
...
from model.net_fre import gtnet_fre#只有频域
...
def test(epoch):
    ...
    for batch_id, (x, z,z2, i, y) in enumerate(test_loader):

        x = torch.unsqueeze(x, dim=1)
        x = Variable(x).float().to(device)
        z = Variable(z).float().to(device)
        z2 = Variable(z2).float().to(device)
        i = Variable(i).float().to(device)
        y = Variable(y).to(device)
        net.eval()
        start_time = time.time()
        # embedding, encoder, output, pred_y = net(x)#需要修改

        _,_, pred_y = net(x, z,z2, i)
...
def train(optimizer):
    ...
    for epoch in range(n_epochs):
        ...
        for batch_id, (x, z,z2, i, y) in enumerate(train_loader):
        ...
        nowtt = time.strftime("%Y-%m-%d-%H_%M_%S", now)
        # save model
        ...
        if total_test_acc > max_accuracy:
            ...
            torch.save(net,
                    ...
                    f'saved_model/{archive}/{archive} batch={args.batch_size} length={length} time={nowtt}.pkl'
...
if __name__ == '__main__':
    ...
    # PEMS-SF
    archive = 'AtrialFibrillation'  ##数据集 1234
    #StandWalkJump
    #CharacterTrajectories
#BasicMotions #跟换数据集且排序后效果明显 图像单独0.4
#HandMovementDirection
#NATOPS 换了高分辨率无白边数据集还是不行
#SelfRegulationSCP2
#ArticularyWordRecognition  换了高分辨率无白边数据集还是不行
#PenDigits
#SpokenArabicDigits #
