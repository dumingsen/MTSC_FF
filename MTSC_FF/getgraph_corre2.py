...
from Teoriginal_corre2 import getTEgraph1
...
dir_name='./corre2graph/{0}'.format(archive_name)
if not os.path.isdir(dir_name):
    os.makedirs(dir_name)

with open('./corre2graph/{0}/{1}_train.txt'.format(archive_name,archive_name), 'w') as outfile:
    for slice_2d in f_g_train:
        np.savetxt(outfile, slice_2d, fmt = '%f', delimiter = ',')
print('写入train')
with open('./corre2graph/{0}/{1}_test.txt'.format(archive_name,archive_name), 'w') as outfile:
    for slice_2d in f_g_test:
        np.savetxt(outfile, slice_2d, fmt = '%f', delimiter = ',')
print('写入test')

c = np.loadtxt('./corre2graph/{0}/{1}_train.txt'.format(archive_name,archive_name), delimiter = ',').\
