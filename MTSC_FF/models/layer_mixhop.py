...
class mixprop(nn.Module):
    ...
    def forward(self, x, adj):
        ...
        out = [h]
        a = adj / d.view(-1, 1)
        print('用可视化热力图',a.shape)
        print(a)
        b = a.unsqueeze(dim=0)
        b = b.unsqueeze(dim=0)

        b = F.interpolate(b,size=1000, mode='nearest')#, size=(100,100)#,scale_factor=20
        now = time.localtime()
        now = time.strftime("%Y-%m-%d-%H_%M_%S", now)
        plt.imsave("linjiejuzhen{}.png".format(now), b.detach().numpy().squeeze())
        #return
