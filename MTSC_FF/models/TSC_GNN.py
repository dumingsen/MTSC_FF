...
class Model(nn.Module):
    ...
    def __init__(self, args, num_class, t=1,  # 长度
        ...
        self.mlp_head = nn.Sequential(
            nn.LayerNorm(t),  ##62,normalized_shape：归一化的维度，int（最后一维）list（list里面的维度）#hidden_dim[-1]
            nn.Linear(t, num_class)  # hidden_dim[-1]
        )
        self.Transformer = Transformer(990, 1, 0.1)

        self.gru = GRU(self.channels, t)

        self.gru1 = GRU(5, 990)

        self.end_linear = nn.Linear(b//2, t)
    ...
    def forward(self, x, x_graph):
        ...
        x3=self.end_linear(x3.to(torch.float32))
        #x3 = self.gru(x3)

        # x3=self.swintransformer(x3)#8, 5, 100
