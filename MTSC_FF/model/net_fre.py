...
class gtnet_fre(nn.Module):
    def __init__(self, gcn_true, buildA_true, gcn_depth, num_nodes, device, predefined_A=None,
        ...
        self.freq_conv3 = nn.Conv2d(in_channels=16, # 图片通道3
                                    ...
                                    bias=True, padding=1)
        #图片尺寸
        self.freq_linear1 = torch.nn.Linear(8 * 64*128, 3072)#4,,,,,,,,,,, 128 * 64 , 256*128
        self.freq_linear2 = torch.nn.Linear(2 * 3072, 3072)
        self.cbam_img = CBAM_img(8, 4, 3)
        self.freq_linear3 = torch.nn.Linear(2 * 3072, 3072)

        #SE
        self.att_fusion=SE_Block(channels, 2)

        #加权
        # self.gate = torch.nn.Linear(3072*self.channels*2, 2)
        # self.gate1 = torch.nn.Linear(3072*self.channels, 2)


        self.relu = nn.ReLU(inplace=True)
        self.leakyrelu=nn.LeakyReLU(inplace=True,)
        self.sigmoid=nn.Sigmoid()
        self.dropout1 = nn.Dropout(p=0.5) #一般，0.5；HMD0.6 0.4;
    ...
    def forward(self, input, graph,graph2, img, idx=None):
        ...
        #预拼接=====================================================================================================
        z=self.dropout2(z)
        im=self.dropout1(self.sigmoid(self.relu(im)))
        #拼接
        #con=torch.cat((z,im),dim=-2)#
        #print('con',con.shape)#[4, 6144, 6])
        ...
        print('x_conv',x_conv.shape)#4, 9, 3072]
        x_conv_time = z.transpose(2, 1)
        # printt('weight img',x_conv)
        # printt('weight time',x_conv_time)
        #input()

        #================================================加权gate
        # x_conv_soft = x_conv.reshape(x_conv.shape[0],-1)
        # x_conv_time_soft = x_conv_time.reshape(x_conv.shape[0],-1)
        
        # # a1=F.softmax(self.gate1(x_conv_soft),dim=-1)
        # # a2=F.softmax(self.gate1(x_conv_time_soft),dim=-1)
        # # printt('a1',a1)
        # # printt('a2',a2)

        # #soft加权
        # gate = F.softmax(self.gate(torch.cat([x_conv_soft, x_conv_time_soft], dim=-1)), dim=-1)
        # print(gate.shape)#[4,  2]
        # print(gate)
        # gatee=torch.max(gate,1)
        # print(gatee)

        # #拼接
        # con = torch.cat([x_conv_soft * gate[:, 0:1], x_conv_time_soft * gate[:, 1:2]], dim=-1)
        # print('con',con.shape)

        # x_conv_soft=x_conv_soft.reshape(x_conv.shape[0],x_conv.shape[1],x_conv.shape[2])
        # x_conv_time_soft=x_conv_soft.reshape(x_conv.shape[0],x_conv.shape[1],x_conv.shape[2])

        # x_conv=x_conv_soft
        # x_conv_time=x_conv_time_soft
        # con = torch.cat([x_conv_soft , x_conv_time_soft ], dim=-1)
        #==========================================================
        #concat 拼接后的全连接
        #con=self.freq_linear3(con)


        #融合数据
        con=con.transpose(2,1)
        ###注意力机制融合
        #con=self.att_fusion(con)
        ...
        if self.decoder == 'GIN':
            #两种模态数据使用不同的图构建原则
            # 图像
            x3 = F.relu(self.gin(con, A_new2))#x_conv con x_conv_time   A_new2
            x3 = x3.squeeze(dim=1)
...
class SE_Block(nn.Module):
    def __init__(self, ch_in, reduction=16):
        ...
        self.avg_pool = nn.AdaptiveAvgPool1d(1)  # 全局自适应池化 2d
    ...
    def forward(self, x):
        ...
        print('y',y.shape)#4,24
        y = self.fc(y).view(b, c, 1) # FC获取通道注意力权重，是具有全局信息的#, 1
        print('y',y.shape)
